...
func (r *UserRepository) UpdateUser(ctx context.Context, userName string, email user.Email) (*user.User, error) {
	var u user.User
	if err := r.db.QueryRow(update_user, userName, email).Scan(&u.UserId, &u.UserName, &u.Email, &u.CreatedAt, &u.UpdatedAt); err != nil {
		return nil, err
	}
	return &user.User{
		UserId:    u.UserId,
